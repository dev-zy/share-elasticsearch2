...
//				client.filterWithHeader(Collections.singletonMap("Authorization", token));
	...
	public String mapping(String index, String type, @SuppressWarnings("rawtypes") Class clazz) throws IOException {
		...
		if(response!=null&&!response.isExists()){
			...
			builder.addCustom(IndexWarmersMetaData.PROTO.fromMap(JSON.parseObject("{settings:"+analyzer(index)+"}")));
			...
			if(sresponse!=null&&!sresponse.getIndexToSettings().isEmpty()){
				for(ObjectCursor<Settings> settings:sresponse.getIndexToSettings().values()){
					...
					if(setting!=null&&setting.getByPrefix("index.analysis.analyzer.es_analyzer").getAsMap().isEmpty()){
						isCustom = false;
					}
				}
			}
